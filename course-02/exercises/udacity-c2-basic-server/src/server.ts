...
(async () => {
  ...
  app.get("/car/:model", (req: Request, res: Response) => {
    let { model } = req.params;

    const car = cars_list.filter((car) => car.model == model);

    //If no ID is found return ID is required
    if (!model) {
      return res.status(400).send("Car model is required");
    }
